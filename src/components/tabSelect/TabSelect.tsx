...
export function TabSelect<T extends string | number>({ options, value, onChange, layoutId = "tab-indicator", size = "md", contained = true, variant = "pill", ariaLabel }: TabSelectProps<T>) {
  const variantClass = variant === "border" ? " es-tab-select--border" : ""
  return (
    <nav className={`es-tab-select es-tab-select--${size}${contained ? "" : " es-tab-select--loose"}${variantClass}`} aria-label={ariaLabel}>
