...
export function BezierCurveEditor({ points, onChange, color = "#126bf9", axisColor = "#cbd3df" }: SplineCurveEditorProps) {
  ...
  return (
    <div className="es-bezier-editor">
      ...
      >
        <div className="es-bezier-editor__grid" />
      ...
      {/* Footer */}
      <div className="es-bezier-editor__footer">
        <div className="es-bezier-editor__coords">
          <span className="es-bezier-editor__coord">
            {sorted.length} point{sorted.length !== 1 ? "s" : ""}
          </span>
          <span className="es-bezier-editor__coord" style={{ opacity: 0.6 }}>
      ...
      {/* Presets */}
      <TabSelect options={PRESET_TAB_OPTIONS} value={matchedPreset} onChange={handlePresetChange} layoutId={presetTabId} size="sm" contained={false} />
    </div>
  )
}
